            ...
            db.collection('portfolios').doc(user()).collection('blogs').get().then(function (querySnapshot) {
                querySnapshot.forEach(function (doc) {
                    ...
                    if (doc.id == 'meta') {
                        ...
                        d.innerHTML = '<br><button class="waves-flat-button btn-raised-primary" onclick="' + updateblogfunc + '">save</button><div style="display: inline-block;" class="dropdown"> <button class="waves-flat-button btn-text-primary dropdown-toggle" type="button" id="dropdownMenuButton' + doc.id.replace(/ /g, "_") + '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Blog Settings </button> <div class="dropdown-menu" aria-labelledby="dropdownMenuButton' + doc.id.replace(/ /g, "_") + '"> <a style="color: black; display: block;" onclick="' + renameBlogFunc + '" class="waves-flat-button dropdown-item" href="#">Change Blog Title</a> <a style="color: black; " onclick="' + deletefunc3 + '" class="waves-flat-button dropdown-item">Delete Blog</a>  </div> </div>'
